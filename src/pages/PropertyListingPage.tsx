...
function PropertyListingPage() {
  ...
  const { data: properties = [], isLoading: propertiesLoading, refetch: refetchProperties } = useQuery<Property[]>(
    ...
    async () => {
      ...
      const url = `http://localhost:5001/api/properties${queryParams.toString() ? "?" + queryParams.toString() : ""}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {properties.filter(property => !tier || property.tier === tier).length > 0 ? (
          properties.filter(property => !tier || property.tier === tier).map((property) => (
            <div
              key={property.id}
