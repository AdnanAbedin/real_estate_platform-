...
function BannerRotator({ placement }: BannerRotatorProps) {
  ...
  const { data: banners = [], isLoading, error } = useQuery<Banner[]>(
    ["banners", placement],
    async () => {
      const response = await axios.get(`http://localhost:5001/api/banners?placement=${placement}`);
      ...
      return response.data.filter(
        (banner:any) =>
          ...
          banner.status === "active" // Ensure only active banners are shown
      );
    },
    {
      onError: (err) => console.error(`Error fetching banners for ${placement}:`, err),
      staleTime: 60000, // Refresh every minute to catch new banners
    }
  );

  // Rotate banners automatically
  useEffect(() => {
    // Only rotate if there are multiple banners
  ...
  if (isLoading) return <div className="text-center py-4">Loading banners...</div>;
  if (error) return <div className="text-center py-4 text-red-500">Error loading banners</div>;
  if (banners.length === 0) {
  ...
  return (
    <div className="banner-container max-w-7xl mx-auto px-4 py-4">
      <a href={currentBanner.targetUrl} target="_blank" rel="noopener noreferrer">
        <img
          src={currentBanner.imageUrl}
          alt={currentBanner.title}
          className="w-full h-32 object-cover rounded-lg hover:opacity-90 transition-opacity"
          onError={(e) => console.error(`Failed to load image: ${currentBanner.imageUrl}`)}
        />
      </a>
    </div>
  );
