...
function AgentProfile({ companyId }: AgentProfileProps) {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery(['whatsappStats', companyId], async () => {
    const response = await axios.get(`http://localhost:5001/api/whatsapp/stats/${companyId}`);
    return response.data;
  });

  // Fetch company details
  const { data: company, isLoading: companyLoading } = useQuery(['company', companyId], async () => {
    ...
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center">
          ...
          <span>Avg Response Time: {stats?.avgResponseTime ? `${Math.round(stats.avgResponseTime)}s` : 'N/A'}</span>
        </div>
        <div className="flex items-center">
          <MessageSquare className="h-5 w-5 text-indigo-600 mr-2" />
